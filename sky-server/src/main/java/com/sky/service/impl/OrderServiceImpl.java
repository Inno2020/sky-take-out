...
public class OrderServiceImpl implements OrderService {
    ...
    public PageResult pageQuery4User(Integer page, Integer pageSize, Integer status) {
        ...
        // 获得查询结果，因为需要返回订单详细信息，因此要遍历封装成OrderVO
        List<OrderVO> list = new ArrayList<>();
        for(Orders orders : pageOrder) {
            Long ordersId = orders.getId();
            List<OrderDetail> orderDetail = orderDetailMapper.getById(ordersId);
    ...
    public void paySuccess(String outTradeNo) {
        ...
        // 通过Websocket向客户浏览器发送推送消息 type orderId content
        Map map = new HashMap();
        map.put("type", 1);
